...
class ApiService {
  ...
  Future<Map<String, dynamic>> createDonation(
    ...
    try {
      ...
      if ((bytes != null && bytes.isNotEmpty) || (filePath != null && filePath.isNotEmpty)) {
        var request = http.MultipartRequest('POST', Uri.parse('$baseUrl/donations'));
        request.headers['Authorization'] = 'Bearer $token';

        donationData.forEach((key, val) {
          if (val != null) {
        ...
        if (bytes != null && bytes.isNotEmpty) {
          final name = fileName ?? 'receipt.png';
  ...
  Future<Map<String, dynamic>> createPayMongoCheckout(Map<String, dynamic> checkoutData) async {
    try {
      String? token = await _getToken();
      final response = await http.post(
        Uri.parse('$baseUrl/payments/paymongo/checkout'),
        headers: {
          'Content-Type': 'application/json',
          'Authorization': 'Bearer $token',
        },
  ...
  Future<Map<String, dynamic>> getDonationHistory() async {
    try {
      ...
      final response = await http.get(
        Uri.parse('$baseUrl/donations?personal=true'),
        headers: {
          'Content-Type': 'application/json',
          'Authorization': 'Bearer $token',
        },
